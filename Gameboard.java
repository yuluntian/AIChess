...
public class Gameboard {
	...
	// Records a move on the gameboard based on the given Move object.
	...
	// Undo move m on the current game board.
	...
	// Determines whether a move is valid given a Move object and the chess pieces' color.
	...
	private boolean findNetwork(int preX, int preY, int x, int y, int color, int count){
		...
		while(node != null){
			...
			if(preX != -1){
				if(preX == x && x == next.x || preY == y && y == next.y || preX-x == preY-y && x-next.x == y-next.y){
